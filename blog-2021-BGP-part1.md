...
1. If an iBGP peer has advertised the route, either the LOCAL_PREF attribute or a custom setting (local to the router)
   is taken as the degree of preference.
   
2. If an eBGP peer has advertised the route, then a custom setting (local to the router) is used to determine the degree
   of preference.
